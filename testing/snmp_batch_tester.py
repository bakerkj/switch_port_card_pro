...
def DoTheLoop(
    sys_descr: str, descr_data: Dict[str, str], type_data: Dict[str, str]
    ...
    try:
        with open(filename, "r", encoding="utf-8") as f:
            for line in f:
                ...
                match_cisco = re.match(
                    r"OID=([\d\.\-]+),\s*Type=(\w+),\s*Value=(.*)", line
                )
...
def batch_test_files(pattern: str = "snmp*.txt", detailed: bool = False) -> None:
    ...
    for res in results:
        print(
            f"{Path(res['file']).name:<35} {res['status']:<8} {res['ports']:<8} {res['copper']:<8} {res['sfp']:<8}"
        )
    ...
    if detailed:
        ...
        for res in results:
            ...
            for lp, info in sorted(res["mapping"].items()):
                ...
                print(
                    f"   {lp:2d} | {info['name']:<25} | {ptype:6s} | {info['if_descr']}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--detailed", "-d", action="store_true", help="Show detailed info"
    )
